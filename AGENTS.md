...
- `src/config.rs` — config loading: legacy single-model format and multi-provider `providers`/`models` format, `COMMA_*` env overrides, API-style auto-detection, `home_dir`, `full_prompt` (explicit system-prompt override, see prompt.rs).
- `src/setup.rs` — `, --setup` interactive configuration wizard (also auto-started on TTY when no provider is configured): manage LLM providers (add/edit/delete/reorder — `models` array order IS the fallback order) and pick the web-search backend. Edits happen in memory; saving merges into the existing JSON (unknown keys preserved, legacy single-model keys upgraded to `providers`+`models`), first copies the old file to `config.json.YYYYMMDD-HHMMSS.bak` (UTC, hand-rolled civil-from-days, no date crate), then writes atomically (temp file + rename). Menus reuse the crossterm raw-mode pattern from `select_command`; field names shown are the literal config keys and stay untranslated.
...
- `uninstall.sh` — removes only the binary (`,`, `,.old`), the response cache (`cache.json`), and the self-update temp dir. User-defined files (`config.json`, `prompt.md`, `additional_prompt.md`, in XDG/legacy/Windows paths) are kept and listed at the end.
...
- The `, --setup` wizard (src/setup.rs) manages LLM providers (order = fallback order) and the search backend, backing up the existing config to a timestamped `.bak` before saving; it starts automatically on TTY when no provider is configured.
