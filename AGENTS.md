...
- `src/prompt.rs` — system prompt loading. The default template is the compiled-in `DEFAULT_PROMPT`, so upgrades always take effect; template selection (`pick_template`): 1) `full_prompt` config key (path to a file — `~/` and config-dir-relative work — or inline template text) = total override; 2) a legacy `prompt.md` (`prompt_path()`, XDG-first) whose content *differs* from `DEFAULT_PROMPT` = honored as a full override (an identical copy is just the installed template and is ignored); 3) otherwise `DEFAULT_PROMPT` with `additional_prompt.md` (`additional_prompt_path()`, same resolution chain) appended when present. `prefer` tool-preference formatting. The `#SEARCH:` rules (`SEARCH_SECTION`) are appended at runtime when `search.provider` is not `off`, so users with a custom prompt get them without editing their file.
...
- `prompt.md` — reference copy of the compiled-in default system prompt (`DEFAULT_PROMPT` in src/prompt.rs — keep the two in sync). **No longer installed** to user config dirs: customization happens via `additional_prompt.md` (appended) or the `full_prompt` config key (total override). Placeholders: `{{SYSTEM_CONTEXT}}`, `{{PREFERENCES}}`.
