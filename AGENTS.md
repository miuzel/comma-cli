...
- `Cargo.toml` — dependencies: `reqwest` (blocking, json, rustls-tls), `serde`, `serde_json`, `sha2` (update checksums), `crossterm`, `rustyline`, `rust-i18n`. TTY detection uses `std::io::IsTerminal` (no `atty` dependency). Release profile: `strip = true`, `opt-level = "z"`, `lto = true` (size-optimized ~3MB binary — keep it that way).
