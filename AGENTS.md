...
- `src/context.rs` — system context gathering (distro, kernel, arch, shell, CWD, user-installed packages) and privacy placeholders (`{{USER}}`, `{{HOSTNAME}}`, `{{HOME}}`) — collected locally, substituted into LLM output only after the response. `get_shell()` reports `$SHELL` when set (Git Bash/MSYS users on Windows), falling back to `/bin/sh` on Unix and `cmd.exe` on Windows.
