...
或在 `, --setup` 中切换。设为 `auto_refine: false` 后命令的执行方式与之前完全一致：stdio 继承，输出实时流式输出。

开启自动精炼时，`,` 需要命令的输出以生成摘要，因此会**捕获**子进程的 stdout/stderr（stdin 仍然继承）：

- 输出在命令结束后才一次性显示，不再是实时流式；
- stdout/stderr 是管道而非 TTY，进度条和颜色可能消失，部分程序会切换为缓冲输出；
- 全屏交互式程序（`vim`、`less` 等）无法正常显示 —— 这类命令请设 `auto_refine: false`，或在 `,` 之外执行。
