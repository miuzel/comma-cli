...
每条生成的命令后都紧跟**恰好一行**提示，告诉你能对它做什么 —— `▸ 下一步：'x' 执行/编辑/精炼，'c' 复制，'q' 退出。` —— 从此不会再出现「命令生成了却不知道怎么执行」。该提示每条命令只打印一次（交互模式下不会重复刷屏），且仅在交互模式出现：一次性（one-shot）与管道 stdin 运行的行为与之前完全一致。
如果执行过的命令退出码非 0，`,` 会自动发起一次精炼：把失败的命令、退出码和截断后的输出摘要发回模型，并打印修正后的命令，仍需你按 `x` 执行（不会自动执行任何命令）。也可以在主提示符直接精炼，无需先执行：
...
在交互式 REPL 中，退出码**非 0** 的命令（含被信号终止）会自动发起一次精炼：把失败的命令、退出码和输出摘要发给模型，并打印修正后的命令 —— 仍需你按 `x` 执行。不会自动执行任何命令，且每条被执行的命令最多触发一次。非 TTY 场景（一次性模式、管道 stdin）与 `COMMA_EVAL_FILE` eval 模式永不自动精炼。
...
- 你的键盘输入会转发给命令，`Ctrl-C` 也会转发（用于中断命令）；
